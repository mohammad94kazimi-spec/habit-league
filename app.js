...
const TODO_CATEGORIES = [
  { key: "work", label: "کاری", color: "#5B8DEF" },
  { key: "personal", label: "شخصی", color: "#4FA37A" },
  { key: "home", label: "خانه", color: "#E3A33B" },
  { key: "health", label: "سلامت", color: "#C4573F" },
  { key: "financial", label: "مالی", color: "#9B7EDE" },
  { key: "other", label: "سایر", color: "#8A8578" },
];
const categoryByKey = (key) => TODO_CATEGORIES.find((c) => c.key === key) || TODO_CATEGORIES[TODO_CATEGORIES.length - 1];
...
function renderTodos() {
  ...
  sorted.forEach((t) => {
    const cat = categoryByKey(t.category);
    const row = document.createElement("div");
    ...
    row.innerHTML = `
      ...
      <div class="todo-body">
        ...
        <div class="todo-meta">
          ...
          ${t.due_date ? `<span class="todo-date">${isoToJalaliLabel(t.due_date)}</span>` : ""}
        </div>
      </div>
...
// ---------- مودال افزودن کار: انتخاب دسته‌بندی + تاریخ شمسی ----------
const todoModalState = { title: "", category: "other", selectedISO: "" , calJY: 0, calJM: 0 };
...
function renderCategoryGrid() {
  ...
  TODO_CATEGORIES.forEach((c) => {
    ...
    chip.type = "button";
    chip.className = "cat-chip" + (todoModalState.category === c.key ? " selected" : "");
    chip.style.setProperty("--chip-color", c.color);
    chip.textContent = c.label;
    chip.addEventListener("click", () => {
      todoModalState.category = c.key;
      renderCategoryGrid();
    });
...
el("todo-modal-save").addEventListener("click", async () => {
  ...
  const { error } = await sb.from("todos").insert({
    ...
    category: todoModalState.category,
    due_date: todoModalState.selectedISO,
  });
