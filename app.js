...
el("day-tasks-list").addEventListener("click", async (e) => {
  ...
  await loadTodos();
  renderTodoCalendar();
  if (currentDayModalIso) openDayTasks(currentDayModalIso);
});
...
el("todo-modal-delete").addEventListener("click", async () => {
  ...
  await sb.from("todos").delete().eq("id", todoModalState.editingId);
  closeTodoModal();
  await loadTodos();
  renderTodos();
  renderTodoCalendar();
...
el("todo-modal-save").addEventListener("click", async () => {
  ...
  if (todoModalState.editingId) {
    ({ error } = await sb.from("todos").update(payload).eq("id", todoModalState.editingId));
  } else {
    ({ error } = await sb.from("todos").insert({ ...payload, user_id: state.user.id }));
  }
