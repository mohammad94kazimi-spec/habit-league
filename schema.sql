...
-- league_members: فقط اعضای همان لیگ می‌بینند
create policy "league_members_select_same_league" on league_members for select using (
  exists (select 1 from league_members m where m.league_id = league_members.league_id and m.user_id = auth.uid())
);
...
create policy "habits_select_own_or_league" on habits for select using (
  ...
  or (league_id is not null and exists (
    select 1 from league_members m where m.league_id = habits.league_id and m.user_id = auth.uid()
  ))
);
